...
async function scrapeGoogleMapsForClientBatch(client, dryRun = false, retryCount = 0) {
  // This is the same as scrapeGoogleMapsForClient but returns { leads: [] } instead of sending email
  // Copy the entire scrapeGoogleMapsForClient function and modify the return
  const MAX_RETRIES = 2;
  ...
  try {
    ...
    try {
      ...
      await log(`⚠️ Initial load slow, continuing anyway...`);
    }

    await page.waitForTimeout(3000);
    await page.waitForSelector('input[id="searchboxinput"]', { timeout: 10000 });

    const searchQuery = `${client.keyword} ${client.targetCity}`;
    await page.fill('input[id="searchboxinput"]', searchQuery);
    ...
    try {
      ...
      await log(`⚠️ No results found for ${client.businessName}`);
...
async function searchYelpAPIBatch(client) {
  ...
  try {
    ...
    await log(`   [YELP API] Found ${businesses.length} businesses`);
